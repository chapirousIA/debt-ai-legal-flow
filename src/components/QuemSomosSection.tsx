      ...
      <div className="container mx-auto px-4">
        <div className="text-center mb-16">
          ...
          <p className={`section-subtitle max-w-3xl mx-auto ${isVisible ? 'animate-fade-in' : 'opacity-0'}`} style={{animationDelay: '200ms'}}>
            O Pedrosa & Peixoto Advogados é especializado na resolução de conflitos fiscais e negociação de dívidas 
            junto à Procuradoria-Geral da Fazenda Nacional, com mais de 15 anos de experiência transformando situações 
            críticas em oportunidades de recomeço para nossos clientes.
          </p>
        </div>

        <div className="mb-12 text-center">
          <h3 className={`text-2xl font-bold text-primary mb-8 ${isVisible ? 'animate-fade-in' : 'opacity-0'}`} style={{animationDelay: '300ms'}}>
            POR QUE NOS ESCOLHER?
          </h3>
        </div>

        <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
          <div className={`glass-card p-6 ${isVisible ? 'animate-fade-in' : 'opacity-0'}`} style={{animationDelay: '400ms'}}>
          ...
          <div className={`glass-card p-6 ${isVisible ? 'animate-fade-in' : 'opacity-0'}`} style={{animationDelay: '500ms'}}>
            <div className="flex items-center mb-4">
              <div className="bg-primary/10 rounded-full p-3 mr-4">
